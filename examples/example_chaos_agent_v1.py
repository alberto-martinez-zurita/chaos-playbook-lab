"""
example_usage_chaos_agent_v10.py
...
from chaos_agent_universal import ChaosAgent, ChaosAgentConfig
...
class OrderAgent:
    ...
    async def execute_order(
        ...
        retry_strategy = self._get_retry_strategy(operation, "400")  # Default to 400
        ...
        while attempt <= max_retries:
            ...
            result = await self.chaos_agent.call(
                ...
                seed=seed + attempt  # Different seed for each retry
            )
    ...
    def _get_retry_strategy(self, tool_name: str, error_code: str) -> Dict[str, Any]:
        """Get retry strategy from playbook for this tool and error code."""
        if not self.playbook:
            return {"max_retries": 0, "backoff_seconds": 1}
        
        for procedure in self.playbook.get("procedures", []):
            if procedure.get("tool") == tool_name and procedure.get("error_code") == error_code:
                return {
...
async def run_experiment(
    ...
    Args:
        config_path: Path to chaos_agent.yaml config file
        playbook_path: Optional path to playbook JSON
        num_iterations: Number of test iterations
        failure_rate: Probability of errors (0.0 to 1.0)
    """
    print("\n" + "="*70)
    ...
    for i in range(num_iterations):
        ...
        seed = 1000 + i  # Unique seed per iteration
...
async def run_comparison():
    ...
    This demonstrates the value of playbooks in improving success rates.
    """
    print("\n" + "="*80)
    ...
    print(f"   Improvement:      {metrics_with['success_rate'] - metrics_without['success_rate']:+6.1f}%")
    
    print(f"\n⏱️  Average Latency:")
    print(f"   Without playbook: {metrics_without['avg_latency_ms']:6.2f} ms")
    print(f"   With playbook:    {metrics_with['avg_latency_ms']:6.2f} ms")
    print(f"   Difference:       {metrics_with['avg_latency_ms'] - metrics_without['avg_latency_ms']:+6.2f} ms")
...
async def main():
    ...
    playbook_path = Path("playbooks/playbook_petstore_default.json")
