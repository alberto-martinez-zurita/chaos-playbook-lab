"""
Dashboard HTML Generator for Parametric Experiments - PHASE 5.2.4 - FINAL v5.1
...
Updates v5.1:
- Chart 4: IMPROVED Cost-Benefit Analysis (ROI) visualization
- Clear separation of Benefit (Success gain) vs Cost (Latency overhead)
- Dual Y-axis with color coding
"""
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="charts-grid">
                    ...
                    <div class="chart-container">
                        <div class="chart-title">ROI Analysis: Benefit vs Cost</div>
        ...
        <div class="footer">
            <p> Agentic Experiment Analysis</p>
        </div>
    </div>
    
    <script>
        // Chart 1: Agent Effectiveness (Bar Chart)
        ...
        // Chart 4: ROI Analysis (UPDATED VISUALIZATION)
        var benefitTrace = {{
            x: {failure_rates_json},
            y: {effectiveness_improvement_json},
            name: 'Benefit: Success Rate Gain (pp)',
            type: 'scatter',
            mode: 'lines',
            fill: 'tozeroy', // Filled area
            fillcolor: 'rgba(78, 205, 196, 0.2)', // Soft green
            line: {{color: '#4ECDC4', width: 3}},
            hovertemplate: '<b>Benefit</b><br>Chaos: %{{x:.0%}}<br>Gain: +%{{y:.1f}}pp<extra></extra>'
        }};
        
        var costTrace = {{
            x: {failure_rates_json},
            y: {latency_overhead_json},
            name: 'Cost: Latency Added (%)',
            type: 'scatter',
            mode: 'lines+markers',
            yaxis: 'y2', // Right axis
            line: {{color: '#FF6B6B', width: 3, dash: 'dot'}}, // Red dotted line
            marker: {{size: 8}},
            hovertemplate: '<b>Cost</b><br>Chaos: %{{x:.0%}}<br>Overhead: +%{{y:.0f}}%<extra></extra>'
        }};

        var combinedLayout = {{
            title: 'ROI Analysis: Benefit (Resilience) vs Cost (Latency)',
            xaxis: {{title: 'Failure Rate (Chaos Level)', tickformat: ',.0%'}},
            yaxis: {{
                title: 'Success Gain (Percentage Points)', 
            ...
            annotations: [{{
                ...
                text: 'Conclusion: High latency cost is paid only when chaos is high, yielding massive resilience gains.',
                showarrow: false,
                font: {{size: 12, color: '#666'}}
            }}]
        }};
        
        Plotly.newPlot('combined-chart', [benefitTrace, costTrace], combinedLayout, {{responsive: true}});
    </script>
</body>
</html>
"""
...
"""
    ...
    latency_html = """
    <div class="table-container">
        <h3>Latency Overhead Rate Summary</h3><BR>
        <table>
            <tr>
                <th>Agent</th>
"""
    ...
    consistency_html = """
    <div class="table-container">
        <h3>Consistency Rate Summary</h3><BR>
        <table>
            <tr>
                <th>Agent</th>
"""
...
def generate_detailed_tables(metrics: Dict) -> str:
    ...
    for rate_str in sorted(metrics.keys(), key=float):
        ...
        html += f"""
        <div class="table-container">
            <h3>Failure Rate: {rate*100:.0f}% ({n_exp} experiments)</h3><BR>
            <table>
                <tr>
                    <th>Metric</th>
...
def extract_chart_data(metrics: Dict):
    ...
    for rate_str in sorted(metrics.keys(), key=float):
        ...
        # Consistency (using safe .get in case key missing in old jsons)
        b_inc = data['baseline'].get('inconsistencies', {}).get('mean', 0.0)
        p_inc = data['playbook'].get('inconsistencies', {}).get('mean', 0.0)
...
def calculate_summary_stats(metrics: Dict):
    ...
    for rate_str in metrics.keys():
        ...
        avg_consistency_playbook += (1.0 - p_inc)
...
def generate_dashboard(metrics_path: Path, output_path: Path):
    ...
    html = HTML_TEMPLATE.format(
        ...
        total_runs=len(chart_data['failure_rates']) * 2 * 20, # estimated
