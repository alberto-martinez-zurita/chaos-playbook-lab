"""
chaos_agent_petstore.py - Chaos injection proxy using Google ADK + OpenAPI (FIXED)

CHANGES FROM ORIGINAL:
- FIX: Download Swagger JSON and cache locally (OpenAPIToolset doesn't support 'url' type)
- Swagger cached in: apis/petstore_swagger.json
- spec_str_type='json' (not 'url')
...
PETSTORE_SWAGGER_URL = "https://petstore3.swagger.io/api/v3/openapi.json"
PETSTORE_SWAGGER_LOCAL = "apis/petstore3_openapi.json"  # Local cache
...
def _download_and_cache_swagger(url: str, local_path: str) -> str:
    ...
    if local_file.exists():
        print(f"   📂 Loading cached Swagger from: {local_path}")
...
def _initialize_chaos_agent() -> tuple[LlmAgent, InMemoryRunner]:
    ...
    # Download and cache Swagger JSON
    try:
        swagger_json_str = _download_and_cache_swagger(
            PETSTORE_SWAGGER_URL,
            PETSTORE_SWAGGER_LOCAL
        )
    except Exception as e:
        print(f"   ❌ Failed to load Swagger: {e}")
        raise
    
    # Create OpenAPIToolset from Swagger JSON STRING (not URL)
    try:
        petstore_toolset = OpenAPIToolset(
            spec_str=swagger_json_str,  # ← STRING, not URL
            spec_str_type='json'        # ← 'json', not 'url'
        )
        print(f"   ✅ OpenAPIToolset loaded ({len(swagger_json_str)} bytes)")
    except Exception as e:
