...
- Add base server URL for API calls
...
AUTHOR: chaos-playbook-engine Phase 3 (ADK API v7 COMPLETE)
DATE: 2025-11-26
"""
...
def _add_server_url(spec_dict: Dict[str, Any], base_url: str) -> Dict[str, Any]:
    """
    Add server URL to OpenAPI spec if missing.
    
    OpenAPI specs need a 'servers' section to know where to make requests.
    
    Args:
        spec_dict: Parsed OpenAPI spec
        base_url: Base URL for the API (e.g., https://petstore3.swagger.io)
    
    Returns:
        dict: Spec with server URL
    """
    cleaned = spec_dict.copy()
    
    # Add servers if missing or empty
    if 'servers' not in cleaned or not cleaned['servers']:
        cleaned['servers'] = [
            {
                "url": base_url,
        ...
        print(f"   ✓ Server URL already present: {cleaned['servers'][0]['url']}")
...
def _download_and_cache_swagger(url: str, local_path: str, base_url: str) -> str:
    ...
    try:
        ...
        cleaned_spec = _add_server_url(cleaned_spec, base_url)
