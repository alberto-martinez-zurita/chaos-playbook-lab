...
FIXED: Clean OpenAPI spec to remove invalid parameters (empty keys)
...
AUTHOR: chaos-playbook-engine Phase 3 (ADK API v3 Fix)
DATE: 2025-11-26
"""
...
PETSTORE_OPENAPI_LOCAL = "apis/petstore3_swagger.json"
...
def _clean_openapi_spec(spec_dict: Dict[str, Any]) -> Dict[str, Any]:
    """
    Clean OpenAPI spec to remove invalid parameters.
    
    Gemini API rejects specs with:
            ...
            for method, operation in methods.items():
                ...
                if 'parameters' in operation:
                    ...
                    for param in operation['parameters']:
                        ...
                        if not param.get('name') or param.get('name') == '':
                            print(f"   ⚠️  Removed parameter with empty name in {method.upper()} {path}")
                            continue
                        ...
                        if 'schema' in param and isinstance(param['schema'], dict):
                            ...
                            # Remove properties with empty keys
                            if 'properties' in schema and isinstance(schema['properties'], dict):
                                cleaned_props = {
                                    k: v for k, v in schema['properties'].items()
                                    if k and k.strip()
                                }
                                if len(cleaned_props) != len(schema['properties']):
                                    print(f"   ⚠️  Removed {len(schema['properties']) - len(cleaned_props)} properties with empty keys")
                ...
                if 'requestBody' in operation:
                    ...
                    if 'content' in request_body:
                        for content_type, content_schema in request_body['content'].items():
                            if 'schema' in content_schema and isinstance(content_schema['schema'], dict):
                                ...
                                if 'properties' in schema and isinstance(schema['properties'], dict):
                                    ...
                                    if len(cleaned_props) != len(schema['properties']):
                                        print(f"   ⚠️  Cleaned requestBody properties in {method.upper()} {path}")
                                    schema['properties'] = cleaned_props
    
    # Clean components/schemas
    if 'components' in cleaned and 'schemas' in cleaned['components']:
        for schema_name, schema in cleaned['components']['schemas'].items():
            if isinstance(schema, dict) and 'properties' in schema:
                if isinstance(schema['properties'], dict):
                    ...
                    if len(cleaned_props) != len(schema['properties']):
                        print(f"   ⚠️  Cleaned schema '{schema_name}' properties")
                    schema['properties'] = cleaned_props
    
    return cleaned


def _download_and_cache_swagger(url: str, local_path: str) -> str:
    """
    Download Swagger JSON from URL and cache locally.
    ...
    Returns:
        str: Swagger JSON as string (cleaned)
    """
    local_file = Path(local_path)
    
    # Check if cached version exists
    if local_file.exists():
        print(f"   📂 Loading cached OpenAPI from: {local_path}")
        ...
        try:
            ...
            print(f"   ✅ Cached Swagger to: {local_path}")
    ...
    try:
        ...
        cleaned_spec = _clean_openapi_spec(spec_dict)
        cleaned_json = json.dumps(cleaned_spec, indent=2)
        print(f"   ✅ Cleaned OpenAPI spec")
        return cleaned_json
...
def _initialize_chaos_agent() -> tuple[LlmAgent, InMemoryRunner]:
    ...
    try:
        ...
        print(f"   💡 Try deleting apis/petstore3_openapi.json and re-running")
        raise
