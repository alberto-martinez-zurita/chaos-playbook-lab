...
- Remove OAuth2 security requirements for public testing
...
AUTHOR: chaos-playbook-engine Phase 3 (ADK API v6 FINAL)
DATE: 2025-11-26
"""
...
def _remove_auth_requirements(spec_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Optionally remove securitySchemes from components
    # (Keep them for documentation, but they won't be enforced)
...
def _download_and_cache_swagger(url: str, local_path: str) -> str:
    ...
    try:
        ...
        print(f"   ✅ Cleaned OpenAPI spec ready (no auth, no problematic endpoints)")
...
async def test_chaos_proxy():
    ...
    if result1.get('body'):
        print(f"   Body preview: {str(result1['body'])[:100]}...")
