...
class PetstoreAgent:
    ...
    def __init__(self, playbook_path: str, mock_mode: bool = None): # Cambiado default a None
        ...
        try:
            ...
            self.config = {"mock_mode": False} # Fallback seguro
    ...
    async def process_order(self, order_id: str, failure_rate: float, seed: int) -> Dict[str, Any]:
        ...
        # ✅ FIX: Pasar mock_mode al ChaosProxy
        ...
        try:
            adk_agent = LlmAgent(
                ...
                CRÍTICO - FINALIZACIÓN:
                - Cuando termines el paso 4, TU ÚNICA SALIDA PERMITIDA es llamar a `report_workflow_success`.
                - ❌ NO escribas "he terminado" ni resúmenes de texto al final.
                - ❌ NO te detengas sin llamar a `report_workflow_success`.
                - Si no llamas a esta herramienta, el trabajo se considera FALLIDO.
...
# Ejecutamos el agente
            ...
            if not self.workflow_succeeded:
                if "completad" in response_text or "success" in response_text:
                     # Opcional: Marcar como éxito parcial o warning
                     print("⚠️ WARNING: Agent claimed success in text but missed tool call.")
            
            duration_ms = (time.time() - start_time) * 1000
            
            status = self.workflow_status if self.workflow_status != "unknown" else "failure"
            ...
            # ✅ FIX: Devolver estructura de fallo limpia en lugar de lanzar excepción
            # Esto permite que el script de comparación continúe con el siguiente experimento
            return {
                "status": "failure",
                "steps_completed": [],
                "failed_at": "exception",
